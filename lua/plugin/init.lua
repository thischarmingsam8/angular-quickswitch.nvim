...
function M.setup(opts)
	...
	vim.api.nvim_create_user_command(
		...
		{ nargs = 0, desc = "Angular: Switch to component" }
	)
	vim.api.nvim_create_user_command(
		"NgQuickSwitchTemplate",
		M.quick_switch_template,
		{ nargs = 0, desc = "Angular: Switch to template" }
	)
	vim.api.nvim_create_user_command(
		"NgQuickSwitchStyles",
		M.quick_switch_styles,
		{ nargs = 0, desc = "Angular: Switch to SCSS" }
	)
	...
	if opts.use_default_keymaps then
		vim.keymap.set(
			"n",
			"<leader>ac",
			":NgQuickSwitchComponent<cr>",
			{ silent = true, desc = "Angular: Switch to component" }
		)
		vim.keymap.set(
			"n",
			"<leader>as",
			":NgQuickSwitchStyles<cr>",
			{ silent = true, desc = "Angular: Switch to SCSS" }
		)
		vim.keymap.set(
			"n",
			"<leader>at",
			":NgQuickSwitchTemplate<cr>",
			{ silent = true, desc = "Angular: Switch to template" }
		)
	end
end
